...
def main():
    ...
    print(
        f"📊 Review lengths - Min: {min(lengths)}, Max: {max(lengths)}, Avg: {sum(lengths)/len(lengths):.1f}"
    )
    ...
    print(
        f"💰 Time Saved:          {trad_time - dyn_time:.2f}s ({((trad_time - dyn_time)/trad_time)*100:.1f}%)"
    )
